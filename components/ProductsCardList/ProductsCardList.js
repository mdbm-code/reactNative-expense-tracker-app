	...
	if (mode === 'three-column') {
		return (
			<View style={[styles.cardRow, styles.card]}>
				{/* Левый блок: картинка */}
				<TouchableOpacity style={styles.imageContainer} onPress={() => onImagePress(product?.images)}>
...
const ProductsCardList = ({
	...
	const { selectedCustomer, selectedProduct, tableOptions, selectedProductManageView } = useSelector(
	...
	const numColumns = selectedProductManageView?.mode === 'single-column'
		? 1
