...
export const selectProducts = createSelector(
	[getSelectedGroup, getProducts, getSelectedCustomer, getCurrentOrders, getSales],
	(selectedMenuLevel_2, productsCatalog, selectedCustomer, currentOrders, sales) => {
		...
		if (Array.isArray(productsCatalog) && productsCatalog.length > 0) {
			...
			if (selectedMenuLevel_2) {
				catalog = catalog.filter(product => product.parentCode === selectedMenuLevel_2); //выводим товары выбранной группы
			} else {
				...
				if (topSalesProductCodes.length > 0) {
					catalog = catalog.filter(item => topSalesProductCodes.includes(item.code));
				} else {
			...
			let toReturn = catalog
				.map(item => {
					...
					return { ...item, price: customerPrice, prices: { default_price: customerPrice, base_price: item.base_price, }, qty: productsQty[item.code] || '' };
				});
